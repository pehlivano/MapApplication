...
public abstract class Graph {
	...
	public List<Integer> degreeSequence() {
		...
		for(int i=0; i<numVertices; i++) {
			//degree of one vertices is equal to sum of its in-degree and out-degree
			int neighbors = getNeighbors(i).size(); // out-degree number
			int inNeighbors = getInNeighbors(i).size(); // in-degree number
			int sum = neighbors + inNeighbors;
			degreeList.add(sum); // add degree to the list
		}
		Comparator c = Collections.reverseOrder();
		Collections.sort(degreeList, c); // Sort the list
		return degreeList;
	}
	...
	public static void main (String[] args) {
		...
		GraphLoader.createIntersectionsFile("data/maps/mecidiyekoy.map", "data/intersections/mecidiyekoy.intersections");

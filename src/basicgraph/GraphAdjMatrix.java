...
public class GraphAdjMatrix extends Graph {
	...
	public List<Integer> getDistance2(int v) {
		...
		//calculate square of adjMatrix
		for (int i = 0; i < n; i++) { // firstRow
            for (int j = 0; j < n; j++) { // secondColumn
                for (int k = 0; k < n; k++) { // firstColumn
		...
		//loop through v'th column
		for(int i=0; i<n; i++) {
			//if number is larger than 0, add multiple times to the list.
